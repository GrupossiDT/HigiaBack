...
class ActualizarAcceso(Form):
    ...
    #ln_id_prgnta = StringField(labels.lbl_id_prgnta, [validators.DataRequired(message=errors.ERR_NO_SN_PRMTRS)])
    lc_cdgo = StringField(labels.lbl_cdgo, [validators.DataRequired(message=errors.ERR_NO_CDGO_PRGNTA)]) 
    lc_dscrpcn = StringField(labels.lbl_prgnta, [validators.DataRequired(message=errors.ERR_NO_PRGTA)])
...
class Preguntas(Resource):
    ...
    def crear(self):
        ...
        if lb_val:
            ...
            lc_datosUsuario = validacionSeguridad.ObtenerDatosUsuario(DatosUsuarioToken['lgn'])[0]
            a_prgnta = {}
            a_prgnta_ge = {}
            ...
            Cursor = Pconnection.querySelect(dbConf.DB_SHMA + '.tbpreguntas_seguridad', 'cdgo', "cdgo='" + str(a_prgnta['cdgo']) + "' or dscrpcn like '%" + str(a_prgnta['dscrpcn']) + "%' ")
            if Cursor :
            ...
            self.crearPregunta_seguridad(a_prgnta_ge, 'tbpreguntas_seguridad_ge')
            
            return Utils.nice_json({"success":labels.SCCSS_RGSTRO_EXTSO, }, 200)
